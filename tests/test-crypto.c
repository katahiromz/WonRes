...
static void TestFailClosed(void)
{
    ...
    if (cbBlob > 0) {
        ...
        CHECK(!WonCryptDecryptBuffer(pbBlob, &pbOut, &cbOut),
              "DecryptBuffer fails on tampered ciphertext (GCM tag mismatch)");
    }
...
////////////////////////////////////////////////////////////////////////////////////
// 3. Nonce uniqueness: encrypting the same plaintext twice must differ
...
static void TestNonceUniqueness(void)
{
    ...
    CHECK(cbBlob1 == 0 || memcmp(pbBlob1, pbBlob2, cbBlob1) != 0,
          "Two encryptions of identical plaintext produce different ciphertext (random nonce)");
